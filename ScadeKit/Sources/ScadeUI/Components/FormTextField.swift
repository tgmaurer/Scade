...
/// SwiftUI's own label layout wins. Inside a `LabeledContent`, with the
/// field's label hidden, it takes effect, which is the same shape the numeric
/// fields already use to get their *trailing* alignment. Those keep it: a
/// column of figures lining up on the right is the point there, and nobody
/// types a space into one.
///
...
struct FormTextField: View {
    ...
    /// field is buried inside a `LabeledContent`, and an identifier on the
    /// container is not the one automation types into.
